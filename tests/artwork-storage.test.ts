...
describe('artwork storage',()=>{
  it('round-trips effects and masks in artwork v2',()=>{const art=createArtwork('FX',()=> 'art'),layer=createLayer(generatorRegistry.get('flow')!,9,palettes[0],()=> 'layer');layer.effects=[{id:'blur-1',effectId:'blur',enabled:true,parameters:{radius:17},order:0}];layer.mask={id:'mask-1',type:'linear-gradient',enabled:true,strength:.7,invert:true,parameters:{angle:42,position:.3,softness:.2}};art.layers=[layer];saveArtwork(storage,generatorRegistry,art);expect(loadArtworks(storage,generatorRegistry)[0]).toEqual(art);});
  ...
  it('migrates a legacy generator preset to a versioned one-layer artwork',()=>{const art=migratePresetToArtwork({name:'Legacy',generator:'flow',seed:77,params:{density:12},palette:palettes[0]},generatorRegistry,()=> 'fixed');expect(art?.version).toBe(2);expect(art?.layers).toHaveLength(1);expect(art?.layers[0]).toMatchObject({generatorId:'flow',seed:77,opacity:1,blendMode:'normal',visible:true});});
