...
export function duplicateArtwork(storage:WriteStorage,registry:GeneratorRegistry,id:string,idFactory:()=>string=createId):Artwork|undefined{const source=loadArtworks(storage,registry).find(item=>item.id===id);if(!source)return;const copy=clone(source);copy.id=idFactory();copy.name=`${source.name} Copy`;copy.layers=copy.layers.map(layer=>({...layer,id:idFactory()}));saveArtwork(storage,registry,copy);return copy;}
