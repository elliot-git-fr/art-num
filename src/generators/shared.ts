...
export const slider = (key: string, label: string, value: number, min: number, max: number, step = 1, random: [number, number] = [min, max], group = 'Geometry'): ParameterDefinition => ({ key, label, type: 'slider', default: value, min, max, step, random, group });
